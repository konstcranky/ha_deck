...
void HdValueCard::set_text_(std::string text) {
    text_ = text;
...
void HdValueCard::render_() {
    ...
    lv_obj_set_style_pad_bottom(lv_main_, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_height(value_container, LV_SIZE_CONTENT);
    lv_obj_set_align(value_container, LV_ALIGN_TOP_MID);

    lv_obj_set_flex_flow(value_container, LV_FLEX_FLOW_ROW);
    lv_obj_set_flex_align(value_container, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_START);
    ...
    lv_obj_set_align(lv_value_, LV_ALIGN_TOP_MID);
    lv_label_set_text(lv_value_, value_.c_str());
    ...
    // lv_obj_set_height(text_container, lv_pct(50));
    lv_obj_set_width(text_container, LV_SIZE_CONTENT);
    ...
    lv_text_ = lv_label_create(text_container);
    lv_obj_set_width(lv_text_, LV_SIZE_CONTENT);
    lv_obj_set_height(lv_text_, LV_SIZE_CONTENT);
    lv_obj_set_align(lv_text_, LV_ALIGN_BOTTOM_MID);
    lv_label_set_text(lv_text_, text_.c_str());
    lv_obj_set_style_text_color(lv_text_, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_opa(lv_text_, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
...
void HdValueCard::update_() {
    if (this->text_fn_) {
        auto s = this->text_fn_();
        if (s.has_value() && s.value() != this->text_)
            this->set_text_(s.value());
    }
...
void HdValueCard::add_text_lambda(std::function<optional<std::string>()> &&f) {
    this->text_fn_ = f;
}
